...
while numInitPts
    ...
    myMean          = single(dataPts(:,stInd));                           % intilize mean to this points location
    myMembers       = [];                                   % points that will get added to this cluster                          
    thisClusterVotes    = zeros(1,numPts,'uint16');         %used to resolve conflicts on cluster membership

    while 1     %loop untill convergence
        
%         tmp = repmat(myMean,1,numPts);
%         tmp = bsxfun(@minus, myMean, dataPts);
%         sqDistToAll = sum((tmp - dataPts).^2);    %dist squared from mean to all points still active
